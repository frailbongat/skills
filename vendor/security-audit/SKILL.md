---
name: security-audit
description: Security guidance and vulnerability review for codebases, APIs, services, CLI tools, libraries, and daemons. Use for security questions, focused security reviews, vulnerability research, security audits, or pen tests. Run the complete workflow only for explicit codebase audit or pen-test requests, full/comprehensive/end-to-end reviews, or requested report artifacts. Not for general code or PR review; use code-review.
disable-model-invocation: true
---

<!--
Vendored from https://github.com/cloudflare/security-audit-skill at commit c1c8a8c
and patched for pi and Paseo: the description, the "pi and Paseo" section, and
the git flag on the "Source ref" line.
The *.test.cjs files were removed. Do not run `npx skills update` on this copy,
because it would overwrite the patch. Re-vendor by hand and re-apply the patch.
-->
...
These rules override the rest of this skill and its companion files where they conflict.

- The Task tool is Paseo `create_agent` with `notifyOnFinish` on. The agent's final message is its result. Read `~/.agents/skills/paseo/SKILL.md` for how to materialize a profile from `list_profiles` into `create_agent`.
- Reconnaissance agents 1a to 1d and any focused reconnaissance agents use `scout`. Coverage critics, Phase 3 verifiers, Phase 5 verifiers, and guidance-mode agents use `reviewer`. In `quick`, Phase 3 and Phase 5 share one merged verifier. Hunters use `delegate`. Never use `researcher`, because it has web access and reconnaissance forbids network access. Never use `worker`.
- Leave the profile's "Follow the role brief in ..." line out of these prompts. Open every hunter, critic, and verifier prompt with this guard line. In a hunter prompt it goes before part 1, the role preamble.

  > The JSON contract at the end of this prompt is your only output format; it overrides any role brief and AGENTS.md. Do not load the security-audit skill. Do not launch sub-agents. No sandbox is approved for this run, so do not build, run, or test target code. Every check is `method: "source"` with `artifact: null`. Run git on the target only as `git -c core.fsmonitor=false`, and add `--no-ext-diff --no-textconv` to `diff`, `show`, and `log -p`.

  Reconnaissance and guidance-mode prompts end in return instructions, not JSON. Open each of them with this guard line:

  > The return instructions at the end of this prompt are your only output format; they override AGENTS.md. Do not load the security-audit skill. Do not launch sub-agents. No sandbox is approved for this run, so do not build, run, or test target code. Run git on the target only as `git -c core.fsmonitor=false`, and add `--no-ext-diff --no-textconv` to `diff`, `show`, and `log -p`.

- Hunter and verifier prompts carry the absolute target path, the output directory, and the canonical `agent_id`. That ID is the skill's agent ID, not the Paseo agent ID. Reconnaissance and critic prompts carry only the absolute target path.
- No OS-enforced sandbox or trusted parent-side promotion code exists here, so no agent runs target code, the parent included. No record can be `confirmed` here, because confirmation needs a bounded local observed result. Every surviving candidate ends as `needs_validation` or `rejected`. When a decisive fact needs runtime observation, the `needs_validation` record's `blockers` name the missing sandbox.
- Record `execution_policy: "source-only"` in `run-metadata.json`. In `REPORT.md`, state that no local check ran, in place of the sandboxed source-and-local-only execution statement.
- Default to `quick` and `"budget": 25` agent invocations, in place of the `standard` default under Run profiles and scope. The user can override either one. If they pick `standard` or `deep` without a budget, propose one under Cost budget before reconnaissance.
...
- **Source ref**: the reviewed commit and whether the worktree is dirty. Do not treat unreviewed generated or modified files as another revision. Run git on the target with `-c core.fsmonitor=false`, and add `--no-ext-diff --no-textconv` to `diff`, `show`, and `log -p`, because the target's `.git/config` can set `core.fsmonitor`, `diff.external`, or a textconv driver to a command that git runs.
